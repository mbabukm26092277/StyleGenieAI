 ...
 * Keeping this as Grounding because it requires deep content understanding (reading the page), not just a link list.
 */
...
/**
 * Helper to perform search using Google Programmable Search Engine API
 */
const performCustomSearch = async (query: string): Promise<GroundingChunk[]> => {
  if (!CSE_ID) {
    ...
    return [{ web: { uri: "#", title: "Configuration Error", snippet: "Search Engine ID (CX) is missing in environment variables." } }];
  }
...
/**
 * Finds nearby salons using Google Programmable Search Engine.
 */
